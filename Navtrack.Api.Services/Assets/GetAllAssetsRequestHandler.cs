...
{
    [Service(typeof(IRequestHandler<GetAllAssetsRequest, IEnumerable<AssetResponseModel>>))]
    public class GetAllAssetsRequestHandler : BaseRequestHandler<GetAllAssetsRequest, IEnumerable<AssetResponseModel>>
    {
        private readonly IMapper mapper;
        private readonly IRepository repository;

        public GetAllAssetsRequestHandler(IMapper mapper, IRepository repository)
        {
            this.mapper = mapper;
            this.repository = repository;
        }

        public override async Task<IEnumerable<AssetResponseModel>> Handle(GetAllAssetsRequest request)
        {
            var entities = await (from asset in repository.GetEntities<AssetEntity>()
                        .Where(x => x.Users.Any(y => y.UserId == request.UserId))
            ...
            List<AssetResponseModel> models = entities
                .Select(x => mapper.Map<AssetEntity, LocationEntity, AssetResponseModel>(x.asset, x.location))
                .ToList();
