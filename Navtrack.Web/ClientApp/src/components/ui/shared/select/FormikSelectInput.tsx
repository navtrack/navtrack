...
export default function FormikSelectInput<T>(props: ISelectInput<T> & IFormikInput) {
  ...
  return (
    <SelectInput
      {...props}
      onChange={(value) => props.onChange ?? formikContext.setFieldValue(props.name, value)}
      onBlur={props.onBlur ?? formikContext.handleBlur}
      label={props.label ? intl.formatMessage({ id: props.label }) : undefined}
      error={fieldMeta && fieldMeta.error && fieldMeta.touched ? fieldMeta.error : undefined}
    />
  );
}
