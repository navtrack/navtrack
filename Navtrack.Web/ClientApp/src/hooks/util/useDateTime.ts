...
export default function useDateTime() {
  ...
  const showDate = useCallback(
    (date?: string): string =>
      date ? moment(date).format("YYYY-MM-DD") : intl.formatMessage({ id: "generic.na" }),
    [intl]
  );

  const showTime = useCallback(
    (date?: string): string =>
      date ? moment(date).format("HH:mm:ss") : intl.formatMessage({ id: "generic.na" }),
    [intl]
  );

  const showDateTime = useCallback(
    (date?: string): string =>
      date ? `${showDate(date)} ${showTime(date)}` : intl.formatMessage({ id: "generic.na" }),
