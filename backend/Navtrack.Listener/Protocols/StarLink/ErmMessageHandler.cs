...
public class ErmMessageHandler : BaseMessageHandler<ErmProtocol>
{
    ...
    public override DeviceMessageEntity? Parse(MessageInput input)
    {
        ...
        if (locationMatch.Success)
        {
            ...
            SetData(locationMatch.Groups[4].Value, deviceMessage);
    ...
    private void SetData(string dataString, DeviceMessageEntity deviceMessage)
    {
        ...
        for (int i = 0; i < Math.Min(data.Length, dataKey.Length); i++)
        {
            if (dataKey[i] == "#LAT#")
            {
                deviceMessage.Latitude = GetCoordinate(data[i]);
            }

            if (dataKey[i] == "#LONG#")
            {
                deviceMessage.Longitude = GetCoordinate(data[i]);
            }

            if (dataKey[i] == "#SPD#")
            {
                deviceMessage.Speed = SpeedUtil.KnotsToKph(Convert.ToSingle(data[i]));
            }

            if (dataKey[i] == "#HEAD#")
            {
                deviceMessage.Heading = Convert.ToSingle(data[i]).ToShort();
            }

            if (dataKey[i] == "#ODO#")
            {
                deviceMessage.DeviceOdometer = Convert.ToInt32(data[i]) * 1000;
            }

            if (dataKey[i] == "#EDT#")
            {
                deviceMessage.Date = DateTimeUtil.Convert(DateFormat.YYMMDDHHMMSS, data[i]);
            }
        }
    }
