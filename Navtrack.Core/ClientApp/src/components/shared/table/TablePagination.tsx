...
import { getRangeArray } from "../../../services/util/NumberUtil";
...
export default function TablePagination<T extends object>(props: Props<T>) {
  const startIndex = (props.currentPage - 1) * props.data.pagination.perPage + 1;
  const endIndex = startIndex + props.data.pagination.perPage - 1;

  const setPage = (page: number): void => {
    if (page >= 1 && page <= props.data.pagination.maxPage) {
  ...
  return (
    <div className="flex justify-center items-center py-3 text-sm select-none">
      <div className="text-xs" style={{ flexGrow: 1, flexBasis: 150 }}>
        <b>{startIndex}</b> to <b>{endIndex}</b> of <b>{props.data.pagination.totalResults}</b>{" "}
        results
      </div>
      <div className="flex justify-center items-center" style={{ flexGrow: 3 }}>
        ...
        {getPages(props.currentPage, props.data.pagination.maxPage).map((x) => (
          <div
            key={x}
